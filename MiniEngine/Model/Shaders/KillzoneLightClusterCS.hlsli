    ...
    uint3 DTid : SV_DispatchThreadID)
{
    ...
    clusterMinDepth = max(2.0f * atan(999.0f * clusterMinDepth) / PI, (1.0f / WORK_GROUP_SIZE_Z) * (Gid.z + 1));
    clusterMaxDepth = min(2.0f * atan(999.0f * clusterMaxDepth) / PI, (1.0f / WORK_GROUP_SIZE_Z) * (Gid.z));
    ...
    float2 invClusterSize2X = float2(ViewportWidth, ViewportHeight) * InvTileDim;
    ...
    float3 clusterBias = float3(
        -2.0 * float(Gid.x) + invClusterSize2X.x - 1.0,
        -2.0 * float(Gid.y) + invClusterSize2X.y - 1.0,
        -clusterMinDepth * invClusterDepthRange);
    float4x4 projToCluster = float4x4(
        invClusterSize2X.x, 0, 0, clusterBias.x,
        0, -invClusterSize2X.y, 0, clusterBias.y,
    ...
    frustumPlanes[0] = clusterMVP[3] + clusterMVP[0];
    frustumPlanes[1] = clusterMVP[3] - clusterMVP[0];
    frustumPlanes[2] = clusterMVP[3] + clusterMVP[1];
    frustumPlanes[3] = clusterMVP[3] - clusterMVP[1];
    frustumPlanes[4] = clusterMVP[3] + clusterMVP[2];
    frustumPlanes[5] = clusterMVP[3] - clusterMVP[2];
    for (int n = 0; n < 6; n++)
    {
